...
namespace CCIA.Helpers
{
    public class LabResultsCheckStandards
    {
        ...
        public string DodderError { get; set; }
        ...
        public static async Task<LabResultsCheckStandards> CheckStandardsFromLabs(CCIAContext _dbContext, SampleLabResults labs)
        {
            ...
            if(labs.GermResults == null || labs.GermResults == "P")
            {
            ...
            if(labs.AssayResults == null || labs.AssayResults == "P")
            {
                ...
                if(cs.Any(c => c.Name == "dodder_grams"))
                {
                    standard = cs.First(c => c.Name == "dodder_grams");
                    if(labs.DodderGrams < standard.MinValue || labs.DodderGrams > standard.MaxValue || labs.DodderGrams == null)
                    {
                        returnList.HasWarnings = true;
                        returnList.DodderError = $"Dodder grams does not fall within crop standard (min {standard.MinValue:n0}, max: {standard.MaxValue:n0})";
                    }
                }
            }
        ...
        public static async Task<LabResultsCheckStandards> CheckStandardsFromBlendLabs(CCIAContext _dbContext, BlendLabResults labs)
        {
            var returnList = new LabResultsCheckStandards();
            var properties = new StandardsProperties();
            int cropId = 0;
